...
apt-get install -y postfix postfix-mysql php5-imap php7.0-imap php7.0-curl
...
sed -i -e "s|\($CONF\['configured'\].*=\).*|\1 true;|"                 \
       ...
       -e "s|\($CONF\['admin_email'\].*=\).*|\1 'postmaster@${DOMAIN}';|"   \
...
echo -e "${CBROWN} - Une adresse email :${CEND} ${CGREEN}postmaster@${DOMAIN}${CEND}"
echo -e "${CBROWN}---------------------------------------------------------------------------${CEND}"
echo ""
...
driver = mysql
connect = host=localhost dbname=postfix user=postfix password=${PFPASSWD}
